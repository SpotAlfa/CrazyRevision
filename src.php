            ...
            foreach ($file as $lineNumber => $line) {
                ...
                if (!empty($matches)) {
                    ...
                    throw new CrazyException(
                        sprintf('Cannot find type declaration in %s on line %d', $filename, $lineNumber)
                    );
                }

                $primitiveTypes = ['int', 'float', 'bool', 'string', 'callable', 'object', 'iterable', 'array'];
                ...
                if (strpos($typeDeclaration, '[]') !== false) {
                    ...
                    $arrayCondition = sprintf(
                        'array_reduce($var, function(bool $carry, $element): bool { return $carry && %s; }, true)',
                        $action
                    );
                }
                ...
                while (true) {
                    ...
                    if (isset($methodName)) {
                        ...
                        /** @noinspection PhpUnhandledExceptionInspection */
                        throw new CrazyException(
                            sprintf('Cannot find method import in %s on line %d', $filename, $lineNumber)
                        );
                    }
                    ...
                    if (isset($conditions)) {
                        ...
                        $conditions = preg_replace_callback(
                            $rangeConditionsPattern, $replaceRangeConditions, $conditions
                        );
                        $conditions = preg_replace_callback(
                            $injectedCodePattern, $replaceInjectedConditions, $conditions
                        );

                        if (isset($arrayCondition)) {
                            $conditions = sprintf('%s && %s', $arrayCondition, $conditions);
                        }
                    } elseif (isset($arrayCondition)) {
                ...
                foreach ($properties as $property) {
                    ...
                    foreach ($imports as $import) {
                        ...
                        switch ($import->methodName) {
                            ...
                            default:
                                $action = sprintf(
                                    '%3$s = $var; return %s::import_%s(%s, $var, ...$args)',
                                    $typeReference,
                                    $import->methodName,
                                    $propertyReference
                                );
                        ...
                        $accessModifiers = ['+' => ZEND_ACC_PUBLIC, '#' => ZEND_ACC_PROTECTED, '-' => ZEND_ACC_PRIVATE];
                        $flags |= $accessModifiers[$import->accessModifier];
